...
export async function searchFeed(rawQuery) {
  ...
  // Housekeeping: drop one-off search rows older than the cache window so the
  // videos table doesn't grow unbounded from searches.
  getDb()
    .prepare(
      `DELETE FROM videos WHERE is_search = 1 AND watched = 0 AND skipped = 0 AND fetched_at < datetime('now', '-${CACHE_HOURS} hours')`
    )
    .run();
